...
\title{Grant Selection Design and Start-Up Innovation: Evidence from Design-Dependent Subsidy Effects}
\subtitle{}
...
\begin{frame}{Master’s thesis}
  \large \highlight{Standardising convertible loan agreements for start-ups}
  \vspace{0.35cm}
 \normalsize
\begin{itemize}
  \item analysed convertible loans as a financing instrument for young firms
  \item compared legal, financial, and contractual design choices
  \item assessed whether standardisation can reduce complexity and transaction costs
  \item developed a model convertible loan agreement 
  \end{itemize}
  \vspace{0.25cm}
  Link to the PhD proposal: how institutional design shapes financing, incentives, and firm behaviour.
\end{frame}

\begin{frame}{Why this PhD topic fits my profile}

\large
My background connects \highlight{firm finance}, \highlight{innovation}, and \highlight{public funding}.
\vspace{0.35cm}
\normalsize
\begin{itemize}
  \item \highlight{Master’s thesis:} empirical work on firm finance and investment
  \item \highlight{agentix GmbH:} Co-Founder and COO of a Munich-based AI automation start-up
  \item \highlight{EXIST grant:} direct exposure to public start-up funding
\end{itemize}
...
\begin{frame}{Identification strategy}
  \large
  Identification uses \highlight{variation in selection rules}.
  \vspace{0.35cm}
  \normalsize
  \begin{itemize}
    \item \highlight{Main:} selection model + panel outcome analysis
    \item \highlight{Cleanest causal design:} RDD at rankings or score cutoffs
    \item \highlight{Extensions:} Diff-in-Diff / event study where timing varies
    \item \highlight{Fallback:} IV only with credible external variation
  \end{itemize}
  \vspace{0.25cm}
  \small
  Goal: estimate the \highlight{selection--outcome alignment gap}.
\end{frame}

\begin{frame}{Expected contribution}
  \large The project contributes to the economics of \highlight{public innovation policy design}.
  \vspace{0.35cm}
  \normalsize
  \begin{itemize}
  \item \highlight{Empirical:} evidence on which selection signals predict public value
  \item \highlight{Conceptual:} separates the selection rule from the grant itself
  \item \highlight{Policy-relevant:} identifies gaps between programme objectives and outcomes
  \end{itemize}
